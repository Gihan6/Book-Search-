...
import com.plcoding.bookpedia.book.prsentation.book_detail.components.BlurredImageBackGround
import com.plcoding.bookpedia.book.prsentation.book_detail.components.BookChip
import com.plcoding.bookpedia.book.prsentation.book_detail.components.ChipSize
import com.plcoding.bookpedia.book.prsentation.book_detail.components.TitleContent
    ...
    BlurredImageBackGround(
        ...
        if (state.book != null) {
            Column(
                ...
                {
                    state.book.averageRating?.let { rate ->

                        TitleContent(title = stringResource(Res.string.rating)) {
                            BookChip {

                                Row {
                                    Text(
                                        text = "${round(rate * 10) / 10.0}"
                                    )
                                    Icon(
                    ...
                    state.book.numPages?.let { pages ->
                        TitleContent(title = stringResource(Res.string.pages)) {
                            BookChip {
                                Text(
                                    text = pages.toString()
                                )
                ...
                if (!state.book.language.isNullOrEmpty()) {
                    TitleContent(
                        ...
                        FlowRow(
                            ...
                            state.book.language.forEach { language ->
                ...
                if (state.isLoading) {
                    ...
                    Text(
                        text = if (state.book.description.isNullOrBlank()) {
                            ...
                            state.book.description?:stringResource(Res.string.description_not_available)
                        },
                        style = MaterialTheme.typography.bodyMedium,
